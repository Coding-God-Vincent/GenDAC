...
exps = ['exp32', 'exp33', 'exp34', 'exp35', 'exp36']
fixed = True
hard_scenario = False
...
for i in range(len(seeds)):
    ...
    def state_preprocessing(pkt_nums, ser_order):
        mean_dict = {
            'volte': 218.8,
            'embb_general': 5338,
            'urllc': 293
        ...
        mean = np.array([mean_dict[ser] for ser in ser_order])
        std = np.array([std_dict[ser] for ser in ser_order])
    ...
    else: env = EnvMove(UE_max_no= UE_no, 
    ...
    for frame in tqdm(range(1, total_timesteps+1)):
        ...
        if changed:
            # remap previous observations stored in LSTM sequence buffer
            lstm_buffer = [
                env.remap_observation_to_current_order(
                    observation=obs,
                    old_order=old_order
                )
                for obs in lstm_buffer
            ]

            print(f"******[Dynamic Profile Shift] frame={frame}, {old_order} -> {new_order}******")
            print("******[LSTM Buffer Remapped]******")
        ...
        with torch.no_grad():
            _ = Model.choose_action(state=state)
        ...
        writer.add_scalar(tag= 'action/dim0', scalar_value= action_space[action][0], global_step= frame)  # 分配比例
        writer.add_scalar(tag= 'action/dim1', scalar_value= action_space[action][1], global_step= frame)
        writer.add_scalar(tag= 'action/dim2', scalar_value= action_space[action][2], global_step= frame)
