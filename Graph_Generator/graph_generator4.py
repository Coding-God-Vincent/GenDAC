...
if fixed: 
    ...
    image_path = Path("/home/super_trumpet/NCKU/Paper/My Methodology/Outcomes/Outcome_fixedUE_env/Combine/others/demand_and_supply_bits/partial") / f"{algo_name}"
else: 
    csv_path = Path("/home/super_trumpet/NCKU/Paper/My Methodology/Outcomes/Outcome_movingUE_env/Combine") / f"{algo_name}_csv"
    image_path = Path("/home/super_trumpet/NCKU/Paper/My Methodology/Outcomes/Outcome_movingUE_env/Combine/others/demand_and_supply_bits/partial") / f"{algo_name}"
...
'''1. (last 2000) observation bits (各網路切片於一個 window 中要傳送的 bits 數) vs supply bits (Action * individual_se -> 各網路切片於該 window 能傳送出多少 bits 數)'''

# embb_general
plt.figure(0)
plt.clf()
plt.title('demand bits vs supply bits - embb_general (last 2000)')
plt.xlabel('Time (s)')
plt.ylabel('bits')
x_axis = range(8000, 10000)
plt.plot(x_axis, ema(actual_ob_embb[-2000:], weight= 0.9), label= 'demand bits', color= 'tab:green', alpha= 0.5)
plt.plot(x_axis, ema(supplybit_embb[-2000:], weight= 0.9), label= 'supply bits', color= 'tab:red', alpha= 0.5)
plt.legend(
    fontsize= 'small',  # 字體大小
...
plt.savefig(image_path / "embb_general_last_2000")


# volte
plt.figure(2)
plt.clf()
plt.title('demand bits vs supply bits - volte (last 2000)')
plt.xlabel('Time (s)')
plt.ylabel('bits')
x_axis = range(8000, 10000)
plt.plot(x_axis, ema(actual_ob_volte[-2000:], weight= 0.9), label= 'demand bits', color= 'tab:green', alpha= 0.5)
plt.plot(x_axis, ema(supplybit_volte[-2000:], weight= 0.9), label= 'supply bits', color= 'tab:red', alpha= 0.5)
plt.legend(
    fontsize= 'small',  # 字體大小
    labelspacing= 0.2,  # 垂直標籤之間的間距
    handletextpad= 0.5,  # 圖示與文字之間的間距
...
plt.savefig(image_path / "volte_last_2000")

# urllc (last 2000 seconds) -> 想看一下細部
plt.figure(3)
plt.clf()
plt.title('demand bits vs supply bits - urllc (last_2000)')
plt.xlabel('Time (s)')
plt.ylabel('bits')
x_axis = range(8000, 10000)
plt.plot(x_axis, ema(actual_ob_urllc[-2000:], weight= 0.9), label= 'demand bits', color= 'tab:green', alpha= 0.5)
plt.plot(x_axis, ema(supplybit_urllc[-2000:], weight= 0.9), label= 'supply bits', color= 'tab:red', alpha= 0.5)
plt.legend(
    fontsize= 'small',  # 字體大小
    labelspacing= 0.2,  # 垂直標籤之間的間距
    handletextpad= 0.5,  # 圖示與文字之間的間距
...
plt.savefig(image_path / "urllc_last_2000")
