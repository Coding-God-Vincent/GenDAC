...
# 環境設定
set_seed(seed= 123)
fixed_UE = True  # True if using fixed UE env, False if moving UE env
if fixed_UE: print("\n================================================== MlpAC fixedUE_env ==================================================\n")
else: print("\n================================================== MlpAC movingUE_env ==================================================\n")
...
for frame in tqdm(range(1, total_timesteps+1)):
    ...
    data = Batch(
        ...
        terminated= False,
        truncated= False,
        obs_next= state_preprocessing(env.get_state()[1])  
    )
    buffer.add(data)
...
plt.figure(5)
plt.clf()
plt.title('Utility (MlpAC Ablation)')
plt.xlabel("Episode")
plt.ylabel("utility")
plt.plot(ma_utility)
