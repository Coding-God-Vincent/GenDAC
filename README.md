...
The TensorBoard event files generated during training can be converted into CSV files using: `CSV_generator_github/csv_generator1.py`

The exported CSV files can then be used by the scripts in `Graph_generator_github/` for further analysis and figure generation.
...
Then specify the random seed and the name of the output experiment:
...
The output directory follows the structure:

```text
Outcome_github/CSVs/seed_<seed>/<algo_name>_csv/
```

For the example above, the generated CSV files are stored in:

```text
Outcome_github/CSVs/seed_124/Test_GenDAC_csv/
```

The directory should be created before running the CSV generator.

### 6.2 Specify the TensorBoard Event File

Set `event_path_moving` to the TensorBoard event file generated by the corresponding experiment.

The TensorBoard event file can be found under:

```text
Logs_github/<algorithm>/<experiment>/tensorboard/
```

For example, for a GenDAC experiment:

```text
Logs_github/GenDAC/exp1/tensorboard/
```

Inside this directory, locate the file whose name starts with `events.out.tfevents`, and use its path in `event_path_moving`.
...
Replace `<experiment>` and the event filename with those of the experiment to be exported.


### 6.3 Generate the CSV Files

After configuring the paths and output settings, run the CSV generator from the project root directory:

```bash
python CSV_generator_github/csv_generator1.py
```

Each TensorBoard scalar specified in `target_tags` will be exported as an individual CSV file. The `/` in TensorBoard tag names is replaced with `_` in the corresponding CSV filename.
...
where `Step` represents the training decision window and `Value` represents the recorded TensorBoard scalar value.
