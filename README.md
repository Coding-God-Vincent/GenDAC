...
Set `event_path_moving` to the TensorBoard event file generated by the corresponding training experiment.
...
Inside the corresponding `tensorboard/` directory, locate the file whose name starts with:

```text
events.out.tfevents
```

and specify its path in `event_path_moving`.
...
Replace the experiment name and TensorBoard event filename with those of the experiment to be exported.
