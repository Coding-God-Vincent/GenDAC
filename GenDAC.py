...
with_or_without = [True, False]
seeds = [123, 124, 125, 126, 127]
exps_w = ['exp18', 'exp14', 'exp15', 'exp16', 'exp17']
exps_wo = ['exp19', 'exp20', 'exp21', 'exp22', 'exp23']
#--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------#
for b in with_or_without:
    if b == True: exps = exps_w
    else: exps = exps_wo
    for i in range(len(seeds)):
        print(f"{b}, seed = {seeds[i]}")
        # 環境參數
        ...
        if fixed_UE: denoise_step = 1  # 1 -> best in fixedUE、5 -> best in movingUE
        else: denoise_step = 5
        actor_lr = 0.001
        critic_lr = 0.001
        weight_decay = 0
