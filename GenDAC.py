...
for fixed in fixed_or_not:
    ...
    for i in range(len(seeds)):
        ...
        def get_actions(state, 
            ...
            if slack_based_explore:  # 用 sigma，不要像 exploration rate 那樣要探索還要看機率。這邊都要探索，大探索或小探索而已。
                ...
                if np.random.rand() < exploration_rate:   
                    noise = to_torch(noise_generator.generate(action_logit.shape, sigma= 0.1), dtype= torch.float32, device= device)  # shape (batch_size, action_dim)  # sigma = 0.1
                    action_logit = action_logit + noise
            ...
            # 為了讓模型輸出極端值，將 action_logits 乘上一個 scale
            # ex: scale = 3.0, 原本輸出的 logit [1, -1, -1] -> [4, -4, -4] 經過 softmax 就可以從 [0.78, 0.11, 0.11] -> [0.995, 0.00247, 0.00247]
            # action_logit = action_logit * action_scale
            ...
            # action_logit = action_logit * 5.0  # 若 clip 完直接進去 softmax，則會沒辦法產生極端的分配方法，故乘上一個 logit
        ...
        # se : np.int with shape (1), qoe : np.array with shape (3)
        # def cal_reward(qoe, se, qoe_weights, se_weight, reward_clipping= False):
        #     standard = 0.95  # standard for embb & volte
        ...
        #--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------#
        # total_band : 20*10^6
        # logit_low : 最低 logit = -0.5
        # logit_hight : 最高 logit = 0.5
        # return : random_logit : np.array with shape (3), real_action : np.array with shape (3)
        def get_random_actions(total_band, logit_low = -0.5, logit_high = 0.5):
        ...
        buffer_size = 10000  # set in GANDDQN
        ...
        for frame in tqdm(range(prefill_steps, total_timesteps)):
            ...
            print(f"action_logits = {action_logit}")
